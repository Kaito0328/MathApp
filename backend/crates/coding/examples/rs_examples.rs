...
fn main() {
    ...
    // 1) 成功する復号（誤り t=2 以内）
    let mut r_ok = c.clone();
    // 2 箇所までの誤りを注入
    r_ok.data[1] = r_ok.data[1].clone() + gf256_from_u8(0x55);
    r_ok.data[6] = r_ok.data[6].clone() + gf256_from_u8(0xAA);
    let dec_ok = rs.decode(&r_ok);
    println!("\n---- decode (<= t errors) ----");
    println!("r: {}", fmt_vec_gf256_hex(&r_ok));
    println!("f': {}", fmt_vec_gf256_hex(&dec_ok.decoded));
    println!("ok? {}", if dec_ok.decoded == f { "true" } else { "false" });

    // 2) 失敗する復号（誤り t を超える）
    let mut r_ng = c.clone();
    // 3 箇所の誤りを注入（t=2 を超える）
    r_ng.data[0] = r_ng.data[0].clone() + gf256_from_u8(0x11);
    r_ng.data[2] = r_ng.data[2].clone() + gf256_from_u8(0x22);
    r_ng.data[5] = r_ng.data[5].clone() + gf256_from_u8(0x33);
    let dec_ng = rs.decode(&r_ng);
    println!("\n---- decode (> t errors) ----");
    println!("r: {}", fmt_vec_gf256_hex(&r_ng));
    println!("f': {}", fmt_vec_gf256_hex(&dec_ng.decoded));
    println!(
        "ok? {} (expected false)",
        if dec_ng.decoded == f { "true" } else { "false" }
    );
}
