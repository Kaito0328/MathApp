...
pub mod error;
pub mod prelude { pub use crate::error::{ConvolutionError, Result as ConvolutionResult}; }
...
pub fn convolve_fft_f64(x: &[f64], h: &[f64]) -> prelude::ConvolutionResult<Vec<f64>> {
    let n = x.len() + h.len() - 1;
    ...
    let x_fft = dft(&x_pad).map_err(|e| prelude::ConvolutionError::FftFailed { text: e.to_string() })?;
    let h_fft = dft(&h_pad).map_err(|e| prelude::ConvolutionError::FftFailed { text: e.to_string() })?;
    let y_fft: Vec<Complex<f64>> = x_fft.into_iter().zip(h_fft).map(|(a, b)| a * b).collect();
    let y = ift(&y_fft).map_err(|e| prelude::ConvolutionError::FftFailed { text: e.to_string() })?;
    Ok(y.into_iter().map(|c| c.re).collect())
}

/// Auto strategy: choose naive or FFT based on work estimate (n*m vs FFT cost)
pub fn convolve_auto_f64(x: &[f64], h: &[f64], threshold: usize) -> prelude::ConvolutionResult<Vec<f64>> {
    let work = x.len() * h.len();
    if work <= threshold {
    Ok(convolve_naive_f64(x, h))
    } else {
