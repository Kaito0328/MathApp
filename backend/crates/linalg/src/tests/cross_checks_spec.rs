use crate::{matrix::numerical::{svd::SvdDeComposition, EigenDecomposition}, Matrix};
...
fn svd_vs_eigen_on_ata() {
    ...
    let mut sig2: Vec<f64> = (0..svd.sigma.dim()).map(|i| svd.sigma[i]*svd.sigma[i]).collect();
    ...
    for (s2,e) in sig2.iter().zip(evals.iter()) { assert!(approx(*s2,*e,1e-6)); }
}
