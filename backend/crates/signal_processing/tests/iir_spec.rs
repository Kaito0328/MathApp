...
fn approx_eq_vec(a: &[f64], b: &[f64], eps: f64) {
...
fn binomial_coeffs_plus_and_minus() {
    ...
    // (x+1)^0 = [1]
    assert_eq!(cache.get_x_plus_1(0), vec![1.0]);
    // (x+1)^3 = [1,3,3,1]
    assert_eq!(cache.get_x_plus_1(3), vec![1.0, 3.0, 3.0, 1.0]);

    // (x-1)^3 = [(-1)^3, 3(-1)^2, 3(-1)^1, 1] with ascending order
    // Ascending coefficients: [ -1, 3, -3, 1 ]
    assert_eq!(cache.get_x_minus_1(3), vec![-1.0, 3.0, -3.0, 1.0]);

    // symmetry check for k=4: (x-1)^4 ascending: [1,-4,6,-4,1]
    assert_eq!(cache.get_x_minus_1(4), vec![1.0, -4.0, 6.0, -4.0, 1.0]);
}
...
fn bilinear_transform_integrator() {
    ...
    let analog_b = vec![1.0];
    let analog_a = vec![0.0, 1.0];
    ...
    let expected_b = vec![0.25, 0.25];
    let expected_a = vec![1.0, -1.0];
...
fn iir_filter_impulse_first_order() {
    ...
    let expected = vec![0.5, 0.25, 0.125, 0.0625, 0.03125];
