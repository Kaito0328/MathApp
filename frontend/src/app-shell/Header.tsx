  ...
  return (
    <View color={CoreColorKey.Primary} size={SizeKey.MD} round={RoundKey.None} style={{ padding: 12 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 12 }}>
        <button aria-label="menu" onClick={onMenu} style={{ justifySelf: 'start', background: 'transparent', border: 'none', color: 'inherit', fontSize: 20 }}>
          ☰
        </button>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, pointerEvents: open ? 'auto' : 'none', zIndex: 1000 }}>
      ...
      <View color={CoreColorKey.Base} size={SizeKey.MD} round={RoundKey.Md}
        style={{ position: 'absolute', top: 0, bottom: 0, left: 0, width: 300, borderRightWidth: 1, transform: `translateX(${open ? 0 : -320}px)`, transition: 'transform 220ms ease', boxShadow: '0 10px 30px rgba(0,0,0,0.25)', padding: 12 }}>
          ...
          {navSections.map((section) => {
            ...
            return (
              <div key={section.title}>
                <button onClick={() => toggle(section.title)} className="nav-toggle">
                  <span aria-hidden style={{ transform: `rotate(${opened ? 90 : 0}deg)`, transition: 'transform 150ms' }}>▶</span>
