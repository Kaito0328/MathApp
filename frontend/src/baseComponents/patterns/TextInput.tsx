...
export const TextInput: React.FC<TextInputProps> = ({ variant = CoreColorKey.Base, size = SizeKey.MD, invalid, className, style, ...rest }) => {
  ...
  return <input className={["input-el", sizeCls, variantCls, invalidCls, className].filter(Boolean).join(' ')} style={{ color: 'inherit', background: 'transparent', ...(style as any) }} {...rest} />;
};
