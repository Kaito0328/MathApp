...
export function VectorSizeControls({ length, onChange, onApply }: { length: number; onChange: (n: number) => void; onApply?: () => void }) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
      <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>n
  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        <label>rows <input type="number" min={1} value={rows} onChange={(e) => onChange(Math.max(1, Math.floor(Number(e.target.value)||1)), cols)} style={{ width: 72 }} /></label>
        <button onClick={() => onChange(rows + 1, cols)}>+</button>
        <button onClick={() => onChange(Math.max(1, rows - 1), cols)}>-</button>
      </div>
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        <label>cols <input type="number" min={1} value={cols} onChange={(e) => onChange(rows, Math.max(1, Math.floor(Number(e.target.value)||1)))} style={{ width: 72 }} /></label>
